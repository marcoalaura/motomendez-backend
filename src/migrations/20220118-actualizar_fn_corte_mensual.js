    ...
    $BODY$
                ...
                BEGIN
                  ...
                  FOR ppcd IN(select pd.id_pcd, p.fecha_nacimiento, pd.cod_municipio
                    ...
                    where ca.fid_gestion = _gestion and (pd.fecha_habilitacion is null or make_date(_gestion, _mes, 20) >= pd.fecha_habilitacion) and pd.estado = 'ACTIVO') 
                  LOOP
                    --Eliminando las pcds que tengan certificado en el IBC
                    -- Se considera hasta el ultimo día del mes
                    ...
                    IF EXISTS(SELECT c.fid_pcd FROM certificado as c WHERE c.fid_pcd = ppcd.id_pcd AND c.fecha_vigencia >= make_date(_gestion, 12, 31) AND c.tipo_certificado = 'IBC') THEN
                    ...
                    ELSIF NOT EXISTS((select fid_pcd from (SELECT fid_pcd, porcentaje_discapacidad, fecha_vigencia FROM certificado WHERE fid_pcd = ppcd.id_pcd AND tipo_certificado = 'SIPRUNPCD' 
                    --Hasta el ultimo día del mes precedente
                    --order by fecha_emision desc limit 1) as a1 where a1.porcentaje_discapacidad >= 50 AND fecha_vigencia >= make_date(_gestion, _mes, 20)) ) then
                    order by fecha_emision desc limit 1) as a1 where a1.porcentaje_discapacidad >= 50 AND fecha_vigencia >= make_date(_gestion, _mes, 1)) ) THEN
                      INSERT INTO corte_mensual_observados(estado, observacion, _usuario_creacion, _fecha_creacion, _fecha_modificacion, fid_gestion, fid_mes, fid_pcd, cod_municipio)
                    ...
                    ELSIF EXISTS (SELECT pbm.fid_pcd from pcd_beneficio_mes AS pbm INNER JOIN beneficio AS b ON(pbm.fid_beneficio = b.id_beneficio) 
                      WHERE b.restriccion = true and pbm.fid_gestion = _gestion and pbm.mes = _mes and pbm.estado = 'ACTIVO' and pbm.fid_pcd = ppcd.id_pcd) then 
                      INSERT INTO corte_mensual_observados(estado, observacion, _usuario_creacion, _fecha_creacion, _fecha_modificacion, fid_gestion, fid_mes, fid_pcd, cod_municipio)
                      VALUES('GENERADO', 'Goza de un beneficio (OVT)', _idUsuario, now(), now(), _gestion, pidmes, ppcd.id_pcd, ppcd.cod_municipio);
