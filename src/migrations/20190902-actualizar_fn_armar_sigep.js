...
module.exports = {
  up: (migration, DataTypes, done) => {
    const sql = (
      `
      CREATE OR REPLACE FUNCTION public.fn_armar_sigep(
        _idusuario integer,
        _gestion integer)
      RETURNS text AS
    $BODY$
           ...
           BEGIN
               ...
               FOR ppcd IN (select p.documento_identidad, p.complemento_documento, p.expedido, p.primer_apellido, p.segundo_apellido, 
                 ...
                 inner join pcd as pd on (ca.fid_pcd = pd.id_pcd and (pd.fecha_habilitacion is null or make_date(_gestion, pmes, 20) >= pd.fecha_habilitacion))
               ...
               LOOP
                  ...
                  IF NOT EXISTS (SELECT documento_identidad FROM log_servicio_sigep WHERE documento_identidad = ppcd.documento_identidad and fecha_nacimiento = ppcd.fecha_nacimiento) THEN
                     ...
                     id_ubigeo, id_entidad_pago, estado, fid_pcd, _usuario_creacion, _fecha_creacion, _fecha_modificacion)
                     ...
                     ppcd.departamento, ppcd.municipio, ppcd.direccion, ppcd.telefono, ppcd.id_ubigeo, ppcd.id_entidad, 'CREADO', ppcd.id_pcd, _idusuario, now(), now());
                  END IF;
             END LOOP;
             return 'Ok';
